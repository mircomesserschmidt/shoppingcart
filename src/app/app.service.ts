...
export class AppService {
  ...
  async removeFromCart(cartId: number, productId: number) {
    if (await this.doesCartExists(cartId) && await this.isProductInCart(cartId, productId)) {
      const res = await this.contentsRepo.findOne({ where: { shoppingCartId: cartId, productid: productId } })
      if (res) {
  ...
  async changeAmountOfProduct(cartId: number, productId: number, amount: number) {
  ...
  private async isProductInCart(cartId: number, productId: number): Promise<boolean> {
    const res = await this.contentsRepo.findOne({
      where: {
        shoppingCartId: cartId, productid: productId
      }
    })
    return res !== null && res !== undefined
  }
